class PhoneBook {
  ...
  search(searchTerm) {
    this.#searchedContacts = this.#contacts.filter((contact) => contact.name.toLowerCase().includes(searchTerm.toLowerCase()));

    this.renderContacts();
  }

  renderContacts() {
    const contactsToDisplay = this.#searchedContacts.length > 0
